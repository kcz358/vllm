...
class Qwen2_5_VLForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def _parse_and_validate_audio_input(
        ...
        if audio_values is None:
            return None
        audio_values = self._validate_and_reshape_mm_tensor(
            audio_values, 'audio_values')
        audio_attention_mask = self._validate_and_reshape_mm_tensor(
            audio_attention_mask, 'audio_attention_mask')
        if not isinstance(audio_values, (torch.Tensor, list)):
            raise ValueError("Incorrect type of audio input features. "
                             f"Got type: {type(audio_values)}")
        return Qwen2AudioInputs(audio_values=audio_values,
                                audio_attention_mask=audio_attention_mask)
    ...
    def get_input_embeddings_v0(
        ...
        audio_input: Optional[tuple[torch.Tensor, ...]] = None,
    ) -> torch.Tensor:
