...
"""Inference-only Aero-Audio model compatible with HuggingFace weights."""
...
from transformers import ProcessorMixin, SequenceFeatureExtractor, Qwen2AudioEncoder
...
# # === Audio Inputs === #
class AeroAudioInputs(TypedDict):
...
class AeroAudioProcessingInfo(BaseProcessingInfo):
...
class AeroAudioDummyInputsBuilder(
        BaseDummyInputsBuilder[AeroAudioProcessingInfo]):
...
class AeroAudioMultiModalProcessor(
        BaseMultiModalProcessor[AeroAudioProcessingInfo]):
    ...
    def _get_prompt_updates(
        ...
        def get_replacement_aero_audio(item_idx: int):
        ...
        return [
            PromptReplacement(
                modality="audio",
                target=audio_token,
                replacement=get_replacement_aero_audio,
            )
        ]


@MULTIMODAL_REGISTRY.register_processor(
    AeroAudioMultiModalProcessor,
    info=AeroAudioProcessingInfo,
    dummy_inputs=AeroAudioDummyInputsBuilder)
class AeroForConditionalGeneration(nn.Module, SupportsMultiModal,
                                         SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.audio_modal_projector = AeroAudioMultiModalProjector(
    ...
    def _parse_and_validate_audio_input(
            self, **kwargs: object) -> Optional[AeroAudioInputs]:
        input_features = kwargs.pop('audio_values', None)
        feature_attention_mask = kwargs.pop('audio_attention_mask', None)
        ...
        return AeroAudioInputs(audio_values=input_features,
                                audio_attention_mask=feature_attention_mask)

    def _process_audio_input(self,
                             audio_input: AeroAudioInputs) -> torch.Tensor:

        input_features = audio_input["audio_values"]
        feature_attention_mask = audio_input["audio_attention_mask"]
